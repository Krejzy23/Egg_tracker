        ...
        <View className="rounded-[28px] bg-white px-2 py-3 shadow-lg">
          <Calendar
            ...
            dayComponent={({ date, state }) => {
              ...
              const eggCount = eggEntries[date.dateString]?.eggs ?? 0;
              const brokenEggCount =
                eggEntries[date.dateString]?.brokenEggs ?? 0;
              const isToday = state === "today";
              const isDisabled = state === "disabled";
              const heatmapColor = getHeatmapColor(eggCount);
              const eggCountTextColor =
                brokenEggCount > 0 ? "text-red-600" : "text-emerald-700";
                ...
                >
                  ...
                  {eggCount > 0 ? (
                    <View className="my-1 flex-row items-center rounded-full bg-white/70 px-1">
                      <Text
                        className={`px-1 text-base font-semibold ${eggCountTextColor}`}
                      >
                        {eggCount}
                      </Text>
