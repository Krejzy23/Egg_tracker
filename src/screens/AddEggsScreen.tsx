...
export default function AddEggsScreen({ route, navigation }: Props) {
  ...
  const { getEggEntryForDate, setEggEntryForDate, chickens } = useEggEntries();
  const { user } = useAuth();
  ...
  const [entryChickens, setEntryChickens] = useState<number>(0);

  useEffect(() => {
    const entry = getEggEntryForDate(date);
  
    setEggs(entry.eggs);
    setBrokenEggs(entry.brokenEggs);
    setEntryChickens(entry.chickens > 0 ? entry.chickens : chickens);
  }, [date, getEggEntryForDate]);

  const handleSave = async () => {
    if (!user) return;

    try {
      setEggEntryForDate(date, {
        eggs,
        brokenEggs,
        chickens: entryChickens,
      });

      await saveEggEntry(user.uid, date, eggs, brokenEggs, entryChickens);
  ...
  return (
    <SafeAreaView className="flex-1 bg-zinc-50">
      <View className="flex-1 px-5 pt-4 pb-10">
        <Header title={t("addEggs.title")} subtitle={t("addEggs.subtitle")} />
        ...
        <View className="rounded-[28px] bg-amber-50 px-5 py-5 shadow-sm">
          ...
          <View className="mt-2 rounded-3xl bg-white px-4 py-5">
            ...
            <Text className="font-medium text-xl text-zinc-500">
              {t("addEggs.chickens")}: {entryChickens}🐔
            </Text>
            ...
            <View className="mt-4 flex-row items-center justify-between">
              ...
              <View className="mx-4 flex-1 items-center rounded-2xl bg-zinc-100 py-2">
                <Text className="text-4xl font-bold text-zinc-900">{eggs}</Text>
              </View>
          ...
          <View className="mt-3 rounded-3xl bg-white px-4 py-5">
            <Text className="text-xl font-medium text-zinc-500">
              {t("addEggs.brokenEggs")}
            </Text>

            <Text className="mt-1 text-sm text-zinc-400">
              {t("addEggs.brokenEggsSubtitle")}
            </Text>

            <View className="mt-4 flex-row items-center justify-between">
              <Pressable
                onPress={() => setBrokenEggs((prev) => Math.max(0, prev - 1))}
                className="h-14 w-14 items-center justify-center rounded-2xl bg-zinc-900"
              >
                <Text className="text-2xl font-bold text-white">-</Text>
              </Pressable>
              ...
              <Pressable
                onPress={() => setBrokenEggs((prev) => prev + 1)}
                className="h-14 w-14 items-center justify-center rounded-2xl bg-zinc-900"
              >
                <Text className="text-2xl font-bold text-white">+</Text>
              </Pressable>
